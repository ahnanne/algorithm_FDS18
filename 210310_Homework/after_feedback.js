...
class DoublyLinkedList {
  constructor() {
    ...
    this.tail = null;
    this.size = 0;
    // 이와 같이 size를 추가적으로 이용하면
    // 시간 복잡도가 평균적으로 절반정도 더 줄어듦.
    // 기본 구현은 아님.
  }

  isEmpty() {
    return !this.head;
    // null로 직접 체크하는 것이 BEST임.
    // falsy값일 경우에도 true로 평가되기 때문.
  }

  prepend(value) { // singly linked list와 사실상 동일
  ...
  append(value) { // tail이 있어서 singly linked list에 비해 유리함.
    const node = new Node(value, null, null);

    if (!this.head) { // isEmpty 활용해도 괜춘
      this.tail = node;
      this.head = node;
    } else {
      this.tail.next = node;
  ...
  insert(index, value) {
    // insert를 구현할 때 size를 활용하면
    // 최악의 경우(=처음부터 끝까지 탐색해야 하는 경우)에서 복잡도를 줄일 수 있음.
    let curr = this.head;
  ...
  printInv() { // print inverse ? -> prev를 잘 구현했는지 확인하기 위해 필요함.
    let curr = this.tail;
